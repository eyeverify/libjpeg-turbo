...
{
  LOGM_S("start_input_pass");
  cinfo->input_iMCU_row = 0;
  start_iMCU_row(cinfo);
  LOGM_E("start_input_pass");
}
...
decompress_onepass (j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  ...
  for (yoffset = coef->MCU_vert_offset; yoffset < coef->MCU_rows_per_iMCU_row;
    ...
    for (MCU_col_num = coef->MCU_ctr; MCU_col_num <= last_MCU_col;
	 MCU_col_num++) {
	  ...
	  LOGM_E("decompress_onepass-susp");
  ...
  if (++(cinfo->input_iMCU_row) < cinfo->total_iMCU_rows) {
    start_iMCU_row(cinfo);
    LOGM_E("decompress_onepass-rc");
    return JPEG_ROW_COMPLETED;
  }
  /* Completed the scan */
  (*cinfo->inputctl->finish_input_pass) (cinfo);
  LOGM_E("decompress_onepass-sc");
...
{
  my_coef_ptr coef;
  LOGM_S("jinit_d_coef_controller");
  ...
  if (cinfo->tile_decode) {
    ...
    LOGM_E("jinit_d_coef_controller");
    return;
  }
#endif
        ...
        huffman_index *index, int current_scan)
{
  ...
  LOGM("consume_data_build_huffman_index_baseline");
        ...
        huffman_index *index, int current_scan)
{
  ...
  JBLOCKROW buffer_ptr;
  jpeg_component_info *compptr;
  LOGM("consume_data_build_huffman_index_progressive");
