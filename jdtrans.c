...
{
  LOGM_S("transdecode_master_selection");
  ...
  LOGM_E("transdecode_master_selection");
}
...
jpeg_build_huffman_index_progressive(j_decompress_ptr cinfo,
        huffman_index *index)
{
  LOGM("jpeg_build_huffman_index_progressive");
  if (cinfo->global_state == DSTATE_READY) {
...
{
  LOGM("jpeg_build_huffman_index_baseline");
  if (cinfo->global_state == DSTATE_READY) {
